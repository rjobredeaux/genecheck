...
[
  ...
  "PosDef", "check for positive definiteness";
]
...
let extract_tactic annot pi =
  match annot.annot_content with 
	...
	List.fold_left (fun res beh ->
	  match res, beh.b_name, beh.b_extended with
	    | Some _, _, _ -> res
	    | None, bname, [s, i, [p]]  
	      when grammar_extension_kwd = s 
	      && grammar_extension_id = i
                && Str.string_match (Str.regexp (".*" ^ bname ^ ".*")) (Property.Names.get_prop_name_id pi) 0
		-> (
...
let extract_tactic = 
  Dynamic.register ~plugin:"local-tactic" "extract_tactic" ~journalize:false (Datatype.func2 Cil_datatype.Code_annotation.ty Property.ty (Datatype.option Datatype.string)) extract_tactic
...
let call_provers () =
  Annotations.iter_all_code_annot
	...
	List.iter (fun ip ->
	  let tactic_opt = extract_tactic annot ip in
