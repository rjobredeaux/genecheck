 ...
 * Copyright (C) 2011-2012  P.L. Garoche and P. Roux
 *
...
let block_v_matrix a b = 
  if a.col <> b.col then
    raise Matrix_dimension_error;
  let co = Array.make_matrix (a.line+b.line) (a.col) (BT.of_int 0) in
  for i= 0 to a.line -1 do
    for j = 0 to a.col -1 do
      co.(i).(j) <- a.content.(i).(j)
    done;
  done;
  for i = 0 to b.line -1 do
    for j = 0 to b.col -1 do
      co.(a.line+i).(j) <- b.content.(i).(j)
    done
  done;
  {line = a.line + b.line;
   col = a.col;
...
let lift_block mat n m pos_l pos_c =
 ...
 let inverse (mat: t) : t =
   Format.eprintf "attempting to invert %a:\n" pp_matrix mat;
   let set_column (mat: t) (column: int) (a: elt array) : unit =
    let n = mat.line and
	p = mat.col and
	m = mat.content  in
    if column <= p then
    begin
      ...
      raise (Failure "Column out of bounds.") in
   let get_column (mat : t) (column: int) : int * elt array =
    let n = mat.line and
	p = mat.col and
	m = mat.content  in
     if column <= p then 
       begin
         let column' = Array.make n (BT.of_int 0) in
         for i = 0 to n - 1 do
    ...
    if n = p then
      (* create augmented t *)
      let augmented = zeros_matrix n (2*n) in
      for i = 1 to n do
        let (dim,col) = get_column mat i in
        let arr = Array.make n (BT.of_int 0) in
        begin
          assert(dim = n);
          arr.(i-1) <- BT.of_int 1;
      ...
      Format.eprintf "just to be sure: augmented matrix: %a" pp_matrix augmented;
      let (_,augmented',_) = gauss_split augmented in
      Format.eprintf "and after gaussian elimination: %a" pp_matrix augmented';
      (* create the inverted matrix and fill in with appropriate values *)
      let inverse = zeros_matrix n n in
      for i = 1 to n do
      ...
      Format.eprintf "found : %a" pp_matrix inverse;
      inverse
    else
      raise Matrix_dimension_error
